...
import optimization
import tokenization
import modeling_subtype as modeling
import metrics
...
flags.DEFINE_string(
    ...
    "The input train data dir.")

flags.DEFINE_string(
    "valid_dir", 'valid.tfrecord',
    "The input valid data dir.")
...
def parse_exmp(serial_exmp):
    ...
    subtype_ids = input_data["subtype_ids"]
    labels = input_data['label_ids']
    return text_a_id, text_b_id, input_sents, input_mask, segment_ids, subtype_ids, labels


def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, subtype_ids, labels, text_a_id, text_b_id,
...
def main(_):
    ...
    train_data_size = total_sample(FLAGS.train_dir[:-9] + "_0.tfrecord")
    ...
    text_a_id, text_b_id, input_sents, input_mask, segment_ids, subtype_ids, labels = iterator.get_next()  # output dir
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(FLAGS.num_train_epochs):
                ...
                sess.run(iterator.initializer,
                         feed_dict={filenames: [FLAGS.train_dir[:-9] + "_" + str(epoch) + ".tfrecord"], shuffle_size: 1024})
