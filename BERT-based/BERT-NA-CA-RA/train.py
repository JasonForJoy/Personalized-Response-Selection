    ...
    input_data = tf.parse_single_example(serial_exmp,
                                       features={
                                           ...
                                           "text_b_id":
                                               tf.FixedLenFeature([], tf.int64),
                                           "input_ids":
                                               tf.FixedLenFeature([FLAGS.max_seq_length*2], tf.int64),
                                           "input_mask":
                                               tf.FixedLenFeature([FLAGS.max_seq_length*2], tf.int64),
                                           "segment_ids":
                                               tf.FixedLenFeature([FLAGS.max_seq_length*2], tf.int64),
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, labels, text_a_id, text_b_id,
  ...
  # print(input_ids.get_shape())    # [batch_size, 2*max_seq_length]
  ...
  '''
  # Method 1: obtain two matching vectors, get two scalar scores and sum them
  ...
  # Method 2: obtain two matching vectors, concatenate them and get a single scalar score
  output_layer = tf.reshape(output_layer, [-1, hidden_size*2])  # [batch_size, hidden_size*2]
